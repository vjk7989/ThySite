/**
 * Icon geometry. Entries hold only what an SVG needs to draw itself
 * (paths, viewBox, stroke/fill defaults). Sizing, colour and state classes are
 * decided at the call site via `<Icon name="…" class="…" />`.
 */
...
export const Icons = {
  ...
  user: {
    paths: [
      ...
      {
        d: 'M12 3a4 4 0 1 0 0 8 4 4 0 1 0 0-8Z',
      },
    ],
    width: 24,
    height: 24,
...
/** For icon names that arrive as plain strings (JSON data): fail the build on a typo. */
export function assertIconName(value: string): IconName {
  if (!isIconName(value)) throw new Error(`Unknown icon "${value}"`);
  return value;
}
